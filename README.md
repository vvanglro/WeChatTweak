...
| **客户端多开** | 同时登录多个账号 | ✓ | 仅 269602：✓；其余 4.x 构建请复制 App |
...
>
> 微信 4.x 的多开通常仍只能整包复制 App。**build 269602 是例外**：本仓库收录的 `multiInstance` 补丁会让单实例检查返回成功；该点已在同 bundle-id 的应用副本并行启动实验中验证。不要让两个实例共享同一账号的数据目录进行长期并发使用。

> **build 269602 同时支持两个防撤回变体**，但二者的静默点不同：`silent` 使用 EchoXml 复核的 `revokemsg` 分类函数 8 字节返回桩；`keeptip` 会先还原这个分类函数，再使用同一 arm64 切片中已验证的 `parseRevokeXML` 几何签名（保留 `cbz`，并把 `newmsgid` 写入改为 0）。两组原始字节均在官方 269602 arm64 切片（SHA-256 `413c1246…a4b70568abd6`）核对；真实聊天撤回仍应自行实测。

## 支持的版本

工具按 **构建号**（`CFBundleVersion`，即 `wechattweak versions` 打印的数字）匹配，不是营销版本号。

当前适配以**微信官网最新稳定版**为准。2026-09-12 核对腾讯更新源为 **4.1.13.63（build 269631）**，已提供 Apple Silicon（arm64）补丁。旧构建 269602 也已收录；其他未收录构建建议从[微信官网](https://mac.weixin.qq.com/)更新到这版，再重新打补丁。官网版和 App Store 版即使都显示 4.1.13，也必须分别核对构建号，不能互套地址。
...
| 269602 | 4.1.13，arm64 | ✓（`silent` 与 `keeptip` 均有原始字节校验；真实收撤回仍待实测） | 运行时定位（不固化地址）；另支持 `multiInstance`（同 bundle-id 副本并行启动验证） |
| 269579 | 4.1.13 | ✓ | ✓ |
| 269136 | 4.1.11 | ✓（本机实测） | ✓ |
