...
> **build 269602 同时支持两个防撤回变体**：`silent` 使用 EchoXml 复核的 `revokemsg` 分类函数 8 字节返回桩；`keeptip` 会先还原该分类函数，再保留 `parseRevokeXML` 的分支并把 `newmsgid` 写入改为 0。两组原始字节均在官方 269602 arm64 切片（SHA-256 `413c1246…a4b70568abd6`）核对；真实聊天撤回仍应自行实测。
